return {
  ...
  config = function()
    ...
    -- First 4 main files
    vim.keymap.set("n", "hf", function() harpoon:list():select(1) end)
    vim.keymap.set("n", "hd", function() harpoon:list():select(2) end)
    vim.keymap.set("n", "hs", function() harpoon:list():select(3) end)
    vim.keymap.set("n", "ha", function() harpoon:list():select(4) end)
    -- Next 4 files (if needed)
    vim.keymap.set("n", "hr", function() harpoon:list():select(5) end)
    vim.keymap.set("n", "he", function() harpoon:list():select(6) end)
    vim.keymap.set("n", "hw", function() harpoon:list():select(7) end)
    vim.keymap.set("n", "hq", function() harpoon:list():select(8) end)
